...
>
> — 계획이 리뷰되었다면, 모든 기술 결정과 그 이유가 `.plan/{plan-name}/`에 남아있다. 코드는 바뀌어도, 결정의 맥락은 사라지지 않는다.
...
**글로벌 하네스** (`.plan/{plan-name}/harness-global.md`): 프로젝트 전체에 일관되게 적용되는 규칙
...
**Step별 하네스** (`.plan/{plan-name}/harness-step-2.md`): 해당 Step의 맥락에 맞는 추가 제약
...
│  범위 결정 → "절대 금지" 항목                             │
│                                                         │
│  → .plan/{plan-name}/divergence-report.md 생성                      │
...
`--calibrate` 실행 후 `.plan/{plan-name}/divergence-report.md`에 생성:
...
하나의 프로젝트에 여러 plan이 공존할 수 있다. 각 plan은 `.plan/{plan-name}/` 하위에 독립적으로 존재한다.
...
- `.plan/{plan-name}/harness-global.md` — 프로젝트 전체 컨벤션 하네스
- `.plan/{plan-name}/harness-step-N.md` — 각 Step의 맥락 하네스
- `.plan/{plan-name}/divergence-report.md` — 교정 시 생성되는 발산 리포트
...
verify() { $DRY_RUN && echo "[DRY] 검증: $1" && return 0; echo "🔍 $1"; eval "$2" || { echo "❌ $1"; echo "$CURRENT_STEP" > ".plan-state-$PLAN_NAME"; exit 1; }; echo "✅ $1"; }
checkpoint() {
  $DRY_RUN && return 0
  local step_harness="$HARNESS_DIR/harness-step-${CURRENT_STEP}.md"
...
# ── Step 1: 프로젝트 스캐폴딩 ──
# 하네스: .plan/{plan-name}/harness-global.md + .plan/{plan-name}/harness-step-1.md
...
# ── Step 2: Google OAuth ──
# 하네스: .plan/{plan-name}/harness-global.md + .plan/{plan-name}/harness-step-2.md
...
# ── Step 3: 팀 생성 ──
# 하네스: .plan/{plan-name}/harness-global.md + .plan/{plan-name}/harness-step-3.md
...
- plan.sh와 `.plan/{plan-name}/` 하네스를 git에 커밋하고 PR로 리뷰할 수 있다
